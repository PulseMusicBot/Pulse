...
public class ImageSize implements SlashCommandComponentHandler {
    ...
    public boolean handle(SlashCommandEvent event) {
        ...
        if (option == null) {
            Messenger.replyTo(event, Embeds.info("%s %s".formatted(setting.getEmoji(), setting.getLabel()), "`%s`".formatted(getChoiceKey(settings.get(setting).toTriState())), Pulse.color(event.getGuild())), 15);
        } else {
            System.out.println(option.getAsString());
            settings.set(setting, Value.of(option.getAsString()));
            Messenger.replyTo(event, Embeds.success("%s Updated".formatted(setting.getLabel()), "`%s`".formatted(getChoiceKey(settings.get(setting).toTriState()))), 15);
        }
        return true;
    }
}
