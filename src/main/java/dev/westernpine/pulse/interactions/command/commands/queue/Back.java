...
public class Back implements SlashCommandComponentHandler {
    ...
    public String description() {
        return "Skip to the previous track.";
    }
    ...
    public boolean handle(SlashCommandEvent event) {
        ...
        if (!controller.voteNext(event.getMember())) {
            controller.setLastChannelId(event.getChannel().getId());
            Messenger.replyTo(event, Embeds.success("Vote to skip to previous track removed.", ""), 15);
            return true;
        }
        ...
        if (current >= needed) {
            controller.setLastChannelId(event.getChannel().getId());
            controller.previousTrack();
            Messenger.replyTo(event, Embeds.success("Skipped to the previous track.", "Enough votes accumulated."), 15);
            return true;
        }

        controller.setLastChannelId(event.getChannel().getId());
        Messenger.replyTo(event, Embeds.success("Voted to skip to previous track.", ""), 15);
        return true;
    }
}
